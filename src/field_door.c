...
static const struct DoorGraphics sDoorAnimGraphicsTable[] =
{
    ...
    {0x3B0,                                                 DOOR_SOUND_SLIDING, DOOR_SIZE_1x2, sDoorAnimTiles_UnusedBattleFrontier, sDoorAnimPalettes_UnusedBattleFrontier},
...
static void DrawDoor(const struct DoorGraphics *gfx, const struct DoorAnimFrame *frame, u32 x, u32 y)
{
    ...
    else
    {
        ...
        } else if (gfx->size == DOOR_SIZE_1x2 || gfx->size == DOOR_SIZE_1x1)
        {
            offset = TILE_OFFSET_4BPP(DOOR_TILE_START_SIZE1);
            size = 8 * TILE_SIZE_4BPP;
        } else {
            DebugPrintfLevel(MGBA_LOG_FATAL, "unsupported door size enum %s, guessing a random offset and size", gfx->size);
            offset = TILE_OFFSET_4BPP(DOOR_TILE_START_SIZE1);
            size = 8 * TILE_SIZE_4BPP;
        }
...
{
    if (gfx == NULL || FieldIsDoorAnimationRunning())
        return -1;
...
void FieldSetDoorOpened(u32 x, u32 y)
{
    ...
    {
        const struct DoorGraphics *gfx = GetDoorGraphics(sDoorAnimGraphicsTable, MapGridGetMetatileIdAt(x, y));
        if (gfx != NULL)
...
s8 FieldAnimateDoorClose(u32 x, u32 y)
{
    ...
    if (gfx->size == DOOR_SIZE_2x2)
        ...
        DebugPrintfLevel(MGBA_LOG_FATAL, "unsupported door size enum %s, guessing a random frames", gfx->size);
        frames = sDoorCloseAnimFrames;
    }
...
{
    return FuncIsActiveTask(Task_AnimateDoor) == TRUE;
}
